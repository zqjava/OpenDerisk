...
```commandline
derisk db migration clean --drop_all_tables -y --confirm_drop_all_tables
```
or 
```commandline
rm -rf pilot/meta_data/alembic/versions/*
rm -rf pilot/meta_data/alembic/derisk.db
```

If your database is a shared database, and you run DERISK in multiple instances, you \
...
and use `derisk db migration` command to manage migration scripts.
"""
...
def _check_database_migration_status(alembic_cfg: AlembicConfig, engine: Engine):
    ...
    If your database is a shared database, and you run DERISK in multiple instances,
...
def _ddl_init_and_upgrade(
    ...
    if disable_alembic_upgrade:
        ...
        warn_msg = (
            ...
            "(`python derisk/app/derisk_server.py "
            "--disable_alembic_upgrade`).\n"
            "we suggest you to use `derisk db migration` to initialize and upgrade "
            "database metadata with alembic, "
            "your can run `derisk db migration --help` to get more information."
        )
