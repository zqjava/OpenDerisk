...
from derisk.util.derisks.base import (
...
class BasePackage(BaseModel):
    ...
    def load_module_class(
        ...
        from derisk.core.awel.dag.loader import _load_modules_from_file
...
class FlowPythonPackage(FlowPackage):
    ...
    @classmethod
    def build_from(cls, values: Dict[str, Any], ext_dict: Dict[str, Any]):
        from derisk.core.awel.dag.loader import _process_modules
...
class ResourcePackage(BasePackage):
    ...
    @classmethod
    def build_from(cls, values: Dict[str, Any], ext_dict: Dict[str, Any]):
        from derisk.agent.resource import Resource
        from derisk.agent.resource.tool.pack import _is_function_tool
...
def parse_package_metadata(package: InstalledPackage) -> BasePackage:
    with open(
        Path(package.root) / DERISKS_METADATA_FILE, mode="r+", encoding="utf-8"
    ) as f:
...
def _load_installed_package(path: str) -> List[InstalledPackage]:
    ...
    for package in os.listdir(path):
        ...
        derisks_metadata_file = full_path / DERISKS_METADATA_FILE
        if (
            full_path.is_dir()
            and install_metadata_file.exists()
            and derisks_metadata_file.exists()
        ):
...
class DERISKsLoader(BaseComponent):
    """The loader of the derisks packages"""

    name: str = "derisk_derisks_loader"

    def __init__(
        self,
        system_app: Optional[SystemApp] = None,
        install_dir: Optional[str] = None,
        load_derisks_interval: int = 10,
    ):
        """Initialize the DERISKsLoader."""
        self._system_app = None
        self._install_dir = install_dir or INSTALL_DIR
        self._packages: Dict[str, BasePackage] = {}
        self._load_derisks_interval = load_derisks_interval
    ...
    def before_start(self):
        ...
        schedule.every(self._load_derisks_interval).seconds.do(self.load_package)
    ...
    def load_package(self, is_first: bool = False) -> None:
        ...
        try:
            ...
            if is_first:
                logger.info(
                    f"Found {len(packages)} derisks packages from {self._install_dir}"
                )
            for package in packages:
                self._packages[package.name] = package
                self._register_packages(package)
        except Exception as e:
            logger.warning(f"Load derisks package error: {e}", e)
