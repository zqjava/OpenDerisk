...
_DEFAULT_REPO = "eosphoros/derisks"
...
def list_repos() -> List[str]:
    ...
    """
    repos = set()
    for repo in os.listdir(DERISKS_REPO_HOME):
        full_path = os.path.join(DERISKS_REPO_HOME, repo)
...
def _get_repo_path(repo: str) -> Path:
    ...
    if len(repo_arr) != 2:
        cl.error(
            ...
            f"eg.(eosphoros/derisks).",
            exit_code=1,
        )
    return Path(DERISKS_REPO_HOME) / repo_arr[0] / repo_arr[1]
...
def _list_repos_details() -> List[Tuple[str, str]]:
    ...
    for repo in repos:
        ...
        full_path = os.path.join(DERISKS_REPO_HOME, repo_group, repo_name)
...
def add_repo(repo: str, repo_url: str, branch: str | None = None):
    ...
    if len(repo_arr) != 2:
        cl.error(
            f"Invalid repo name '{repo}', repo name must split by '/', "
            "eg.(eosphoros/derisks).",
            exit_code=1,
        )
    repo_name = repo_arr[1]
    repo_group_dir = os.path.join(DERISKS_REPO_HOME, repo_arr[0])
...
def update_repo(repo: str):
    ...
    repo_path = os.path.join(DERISKS_REPO_HOME, repo)
    if not os.path.exists(repo_path):
...
def install(
    ...
    """Install the specified derisk from the specified repo

    Args:
        name (str): The name of the derisk
        repo (str): The name of the repo
    ...
    if not repo_info:
        cl.error(f"The specified derisk '{name}' does not exist.", exit_code=1)
    repo, derisk_path = repo_info
    copy_and_install(repo, name, derisk_path)


def uninstall(name: str):
    """Uninstall the specified derisk
    ...
    if not install_path.exists():
        cl.error(f"The derisk '{name}' has not been installed yet.", exit_code=1)
    os.chdir(install_path)
    subprocess.run(["pip", "uninstall", name, "-y"], check=True)
    shutil.rmtree(install_path)
    cl.info(f"Uninstalling derisk '{name}'...")
...
def reinstall(
    ...
    """Reinstall the specified derisk
...
def inner_uninstall(name: str):
    ...
    install_path = INSTALL_DIR / name
    if not install_path.exists():
        logger.warning(f"The derisk '{name}' has not been installed yet.")
        return
    os.chdir(install_path)
    # subprocess.run(["pip", "uninstall", name, "-y"], check=True)
    process = subprocess.Popen(
    ...
    shutil.rmtree(install_path)
    logger.info(f"Uninstalling derisk '{name}'...")


def inner_copy_and_install(repo: str, name: str, package_path: Path):
    if not package_path.exists():
        raise ValueError(
            f"The specified derisk '{name}' does not exist in the {repo} tap."
        )
    install_path = INSTALL_DIR / name
    if install_path.exists():
        logger.info(
            f"The derisk '{name}' has already been installed"
    ...
    try:
        ...
        logger.info(f"Installing derisks '{name}' from {repo}...")
        ...
        logger.info(
            f"Installing derisks '{name}' wheel file {_print_path(wheel_file)}..."
        )
        ...
        logger.info(f"Installed derisks at {_print_path(install_path)}.")
        logger.info(f"derisks '{name}' installed successfully.")
        return True
    except Exception as e:
        if install_path.exists():
            shutil.rmtree(install_path)
...
def copy_and_install(repo: str, name: str, package_path: Path):
    """Install the specified derisk from a repository
    ...
    try:
        shutil.copytree(package_path, install_path)
        cl.info(f"Installing derisks '{name}' from {repo}...")
        ...
        wheel_file = wheel_files[0]
        cl.info(f"Installing derisks '{name}' wheel file {_print_path(wheel_file)}...")
        ...
        _write_install_metadata(name, repo, install_path)
        cl.success(f"Installed derisks at {_print_path(install_path)}.")
        cl.success(f"derisks '{name}' installed successfully.")
    except Exception as e:
        if install_path.exists():
            shutil.rmtree(install_path)
        raise e
...
def check_with_retry(
    ...
    """Check the specified derisk with retry.

    Args:
        name (str): The name of the derisk
        spec_repo (str): The name of the repo
        with_update (bool): Whether to update the repo before installing
        is_first (bool): Whether it's the first time to check the derisk
    Returns:
        Tuple[str, Path] | None: The repo and the path of the derisk
    """
    repos = _list_repos_details()
    if spec_repo:
    ...
    for repo in repos:
        ...
        for package in DEFAULT_PACKAGES:
            derisk_path = repo_path / package / name
            derisk_metadata_path = derisk_path / DERISKS_METADATA_FILE
            if (
                derisk_path.exists()
                and derisk_path.is_dir()
                and derisk_metadata_path.exists()
            ):
                return repo[0], derisk_path
    if is_first:
...
def list_repo_apps(repo: str | None = None, with_update: bool = True):
    ...
    if with_update:
        for repo in repos:
            update_repo(repo[0])
    table = Table(title=_("derisks In All Repos"))
    ...
    for repo in repos:
        repo_path = Path(repo[1])
        for package in DEFAULT_PACKAGES:
            derisk_path = repo_path / package
            for app in os.listdir(derisk_path):
                derisk_metadata_path = derisk_path / app / DERISKS_METADATA_FILE
                if (
                    derisk_path.exists()
                    and derisk_path.is_dir()
                    and derisk_metadata_path.exists()
...
def list_installed_apps():
    ...
    table = Table(title=_("Installed derisks"))
