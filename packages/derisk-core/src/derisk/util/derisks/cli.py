...
def add_tap_options(func):
    @click.option(
        ...
        default=None,
        required=False,
        help="The repository to install the derisks from",
    )
...
def install(repo: str | None, update: bool, names: list[str]):
    """Install your derisks(operators,agents,workflows or apps)"""
...
def uninstall(names: list[str]):
    """Uninstall your derisks(operators,agents,workflows or apps)"""
...
def reinstall(repo: str | None, update: bool, names: list[str]):
    """Reinstall your derisks(operators,agents,workflows or apps)"""
...
@click.option(
    ...
    type=str,
    required=True,
    help="The name you want to give to the derisk",
)
@click.option(
    "-l",
    "--label",
...
@click.option(
    ...
    default=None,
    required=False,
    help="The description of the derisk",
)
@click.option(
    "-t",
    "--type",
...
@click.option(
    ...
    default="json",
    required=False,
    help="The definition type of the derisk",
)
@click.option(
    "-C",
    "--directory",
    ...
    default=None,
    required=False,
    help="The working directory of the derisk(defaults to the current directory).",
)
def new_derisks(
    name: str,
    label: str | None,
    ...
    """New a derisks module structure"""
    if not label:
        # Set label to the name
        default_label = name.replace("-", " ").replace("_", " ").title()
        label = click.prompt(
            "Please input the label of the derisk", default=default_label
        )
    if not description:
        # Read with click
        description = click.prompt(
            "Please input the description of the derisk", default=""
        )
    if not directory:
        # Set directory to the current directory(abs path)
        directory = click.prompt(
            "Please input the working directory of the derisk",
