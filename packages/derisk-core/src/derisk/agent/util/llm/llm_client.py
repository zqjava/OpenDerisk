...
class AIWrapper:
    ...
    async def create(self, **config):
        ...
        # 【修复】如果 ModelConfigCache 为空，尝试从配置加载
        if not ModelConfigCache.get_all_models():
            try:
                ...
                if agent_llm_conf and agent_llm_conf.providers:
                    ...
                    if model_configs:
                        ...
                        logger.info(
                            f"[LLMClient] 从 ConfigManager 加载了 {len(model_configs)} 个模型配置"
                        )
                        ...
                        if agent_conf and isinstance(agent_conf, dict):
                            ...
                            if agent_llm_conf:
                                ...
                                if model_configs:
                                    ModelConfigCache.register_configs(model_configs)
                                    logger.info(
                                        f"[LLMClient] 从 ConfigManager.agent.llm 加载了 {len(model_configs)} 个模型配置"
                                    )
            except Exception as e:
                logger.warning(f"[LLMClient] 动态加载模型配置失败: {e}")
