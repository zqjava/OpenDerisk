...
def _dev_mode() -> bool:
    ...
    the same process with the DERISK webserver.
    """
...
class BaseOperator(DAGNode, ABC, Generic[OUT], metaclass=BaseOperatorMeta):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not runner:
            from derisk.core.awel import DefaultWorkflowRunner
    ...
    async def call(
        ...
        with root_tracer.start_span("derisk.awel.operator.call"):
    ...
    async def call_stream(
        ...
        if call_data != EMPTY_DATA:
            call_data = {"data": call_data}
        with root_tracer.start_span("derisk.awel.operator.call_stream"):
            out_ctx = await self._runner.execute_workflow(
                self,
                call_data,
                streaming_call=True,
            ...
            return root_tracer.wrapper_async_stream(
                stream_generator, "derisk.awel.operator.call_stream.iterate"
            )
