...
from derisk.core.interface.message import ModelMessage
...
from derisk.model.parameter import ModelAPIServerParameters, WorkerType
from derisk.util.chat_util import transform_to_sse
...
class APIServer(BaseComponent):
    ...
    async def get_available_models(self) -> ModelList:
        ...
        for m in models:
            model_cards.append(
                ModelCard(
                    id=m, root=m, owned_by="DERISK", permission=[ModelPermission()]
                )
            )
    ...
    async def embeddings_generate(
        ...
        with root_tracer.start_span(
            "derisk.model.apiserver.generate_embeddings",
...
async def create_chat_completion(
    ...
    trace_kwargs = {
        "operation_name": "derisk.model.apiserver.create_chat_completion",
        "metadata": {
            "model": request.model,
...
async def create_completion(
    ...
    trace_kwargs = {
        "operation_name": "derisk.model.apiserver.create_completion",
...
async def create_relevance(
    ...
    with root_tracer.start_span(
        "derisk.model.apiserver.generate_relevance",
        metadata={
            "model": request.model,
            "query": request.query,
        },
...
def _initialize_all(controller_addr: str, system_app: SystemApp):
    ...
    from derisk.model.cluster.worker.manager import _DefaultWorkerManagerFactory
...
def initialize_apiserver(
    ...
    trace_file = trace_config.file or os.path.join(
        "logs", "derisk_model_apiserver_tracer.jsonl"
    )
    initialize_tracer(
        trace_file,
        system_app=system_app,
        root_operation_name=trace_config.root_operation_name or "DERISK-APIServer",
