...
async def create(
    ...
    """Create a new DerisksHub entity
...
async def source_refresh(
    ...
    try:
        ...
        logger.error("Derisks hub source refresh Error!", e)
        return Result.failed(err_code="E0020", msg=f"Derisks Hub refresh Error! {e}")


@router.post("/install", response_model=Result[str])
async def install(request: ServeRequest):
    logger.info(f"derisks install:{request.name},{request.type}")

    try:
        from derisk_serve.derisks.my.config import (
            SERVE_SERVICE_COMPONENT_NAME as MY_GPTS_SERVICE_COMPONENT,
        )
        from derisk_serve.derisks.my.service.service import Service as MyGptsService
        ...
        await blocking_func_to_async(
            ...
            derisk_path=request.storage_url,
            user_name=None,
            sys_code=None,
        )
        return Result.succ(None)
