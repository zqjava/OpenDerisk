...
from derisk.util.derisks.repo import _install_default_repos_if_no_repos, list_derisks
...
class Service(BaseService[ServeEntity, ServeRequest, ServerResponse]):
    """The service class for DerisksHub"""
    ...
    def get_list(self, request: ServeRequest) -> List[ServerResponse]:
        """Get a list of DerisksHub entities
    ...
    def get_list_by_page(
        ...
        """Get a list of DerisksHub entities by page
        ...
        return self.dao.derisks_list(query_request, page, page_size)
    ...
    def refresh_hub_from_git(
        ...
        data: List[Tuple[str, str, str, str]] = list_derisks()
        ...
        try:
            for repo, package, name, gpts_path in data:
                try:
                    if not name:
                        logger.info(
                            f"derisks error repo:{repo}, package:{package}, name:{name},"
                    ...
                    if old_hub_info:
                        ...
                        request.author = self._get_derisks_author(base_package.authors)
                        request.email = self._get_derisks_email(base_package.authors)
    ...
    def _get_derisks_author(self, authors):
        pattern = r"(.+?)<"
        names = []
        for item in authors:
    ...
    def _get_derisks_email(self, authors):
        pattern = r"<(.*?)>"
        emails: List[str] = []
        for item in authors:
