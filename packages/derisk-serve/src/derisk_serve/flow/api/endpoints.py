...
async def export_flow(
    uid: str,
    export_type: Literal["json", "derisks"] = Query(
        "json", description="export type(json or derisks)"
    ),
    ...
    elif export_type == "derisks":
        from ..service.share_utils import _generate_derisks_zip

        if format == "json":
            raise HTTPException(
                status_code=400, detail="json response is not supported for derisks"
            )

        zip_buffer = await blocking_func_to_async(
            global_system_app, _generate_derisks_zip, package_name, flow
        )
        return StreamingResponse(
            zip_buffer,
