...
def _generate_derisks_zip(package_name: str, flow: ServeRequest) -> io.BytesIO:
    ...
    with zipfile.ZipFile(zip_buffer, "a", zipfile.ZIP_DEFLATED, False) as zip_file:
        ...
        # Create derisks.toml
        derisks_toml = tomlkit.document()
        # Add flow information
        derisks_flow_toml = tomlkit.document()
        derisks_flow_toml.add("label", flow_label)
        name_with_comment = tomlkit.string(flow_name)
        name_with_comment.comment("A unique name for all derisks")
        derisks_flow_toml.add("name", name_with_comment)
        derisks_flow_toml.add("version", "0.1.0")
        derisks_flow_toml.add(
            "description",
            flow_description,
        )
        derisks_flow_toml.add("authors", [])
        ...
        derisks_flow_toml.add("definition_type", definition_type_with_comment)
        derisks_toml.add("flow", derisks_flow_toml)

        # Add python and json config
        python_config = tomlkit.table()
        derisks_toml.add("python_config", python_config)
        json_config = tomlkit.table()
        json_config.add("file_path", "definition/flow_definition.json")
        json_config.comment("Json config")
        derisks_toml.add("json_config", json_config)

        # Transform derisks.toml to string
        toml_string = tomlkit.dumps(derisks_toml)
        zip_file.writestr(f"{package_name}/derisks.toml", toml_string)
        ...
        project_section.add("description", flow_description or "A derisks package")
...
async def _parse_flow_from_zip_file(
    ...
    from derisk.util.derisks.loader import _load_flow_package_from_zip_path
