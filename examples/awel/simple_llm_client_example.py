"""AWEL: Simple llm client example
    ...
    Examples:
        ...
        .. code-block:: shell
            ...
            MODEL="gpt-3.5-turbo"
            curl -X POST $DERISK_SERVER/api/v1/awel/trigger/examples/simple_client/chat/completions \
        ...
        Call model and count token.
         .. code-block:: shell
            ...
            curl -X POST $DERISK_SERVER/api/v1/awel/trigger/examples/simple_client/count_token \
            -H "Content-Type: application/json" -d '{
                "model": "'"$MODEL"'",
                "messages": "hello"
            }'
...
from derisk.core.awel import DAG, BranchJoinOperator, HttpTrigger, MapOperator
from derisk.core.operators import LLMBranchOperator, RequestBuilderOperator
...
with DAG("derisk_awel_simple_llm_client_generate") as client_generate_dag:
...
with DAG("derisk_awel_simple_llm_client_count_token") as client_count_token_dag:
    # Receive http request and trigger dag to run.
    trigger = HttpTrigger(
...
if __name__ == "__main__":
    if client_generate_dag.leaf_nodes[0].dev_mode:
        ...
        # Production mode, DERISK will automatically load and execute the current file after startup.
        pass
