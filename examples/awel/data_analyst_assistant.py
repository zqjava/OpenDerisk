"""AWEL: Data analyst assistant.
    ...
    DERISK will automatically load and execute the current file after startup.
    ...
    Examples:
        ...
        .. code-block:: shell
            ...
            DERISK_SERVER="http://127.0.0.1:5555"
            MODEL="gpt-3.5-turbo"
            # Fist round
            curl -X POST $DERISK_SERVER/api/v1/awel/trigger/examples/data_analyst/copilot \
            -H "Content-Type: application/json" -d '{
                "command": "derisk_awel_data_analyst_code_fix",
                "model": "'"$MODEL"'",
                "stream": false,
                "context": {
...
from derisk.model.operators import (
    LLMOperator,
    OpenAIStreamingOutputOperator,
    StreamingLLMOperator,
)
from derisk_serve.conversation.operators import ServePreChatHistoryLoadOperator
...
CODE_DEFAULT = "derisk_awel_data_analyst_code_default"
CODE_FIX = "derisk_awel_data_analyst_code_fix"
CODE_PERF = "derisk_awel_data_analyst_code_perf"
CODE_EXPLAIN = "derisk_awel_data_analyst_code_explain"
CODE_COMMENT = "derisk_awel_data_analyst_code_comment"
CODE_TRANSLATE = "derisk_awel_data_analyst_code_translate"
...
class PromptTemplateBuilderOperator(MapOperator[TriggerReqBody, ChatPromptTemplate]):
    ...
    async def map(self, input_value: TriggerReqBody) -> ChatPromptTemplate:
        from derisk_serve.prompt.serve import SERVE_APP_NAME as PROMPT_SERVE_APP_NAME
        from derisk_serve.prompt.serve import Serve as PromptServe
...
with DAG("derisk_awel_data_analyst_assistant") as dag:
    trigger = HttpTrigger(
    ...
    keep_start_rounds = int(os.getenv("DERISK_AWEL_DATA_ANALYST_KEEP_START_ROUNDS", 0))
    keep_end_rounds = int(os.getenv("DERISK_AWEL_DATA_ANALYST_KEEP_END_ROUNDS", 5))
