...
fmt: setup ## Format Python code
	...
	$(VENV_BIN)/ruff check --fix packages \
		--exclude="packages/derisk-serve/src/**"

	$(VENV_BIN)/ruff check --fix packages/derisk-serve --ignore F811,F841
...
test: $(VENV)/.testenv ## Run unit tests
	$(VENV_BIN)/pytest derisk

.PHONY: test-doc
test-doc: $(VENV)/.testenv ## Run doctests
	# -k "not test_" skips tests that are not doctests.
	$(VENV_BIN)/pytest --doctest-modules -k "not test_" derisk/core

.PHONY: mypy
mypy: $(VENV)/.testenv ## Run mypy checks
	# https://github.com/python/mypy
	$(VENV_BIN)/mypy --config-file .mypy.ini derisk/rag/ derisk/datasource/ derisk/client/ derisk/agent/ derisk/vis/ derisk/experimental/
	# rag depends on core and storage, so we not need to check it again.
	# $(VENV_BIN)/mypy --config-file .mypy.ini derisk/storage/
	# $(VENV_BIN)/mypy --config-file .mypy.ini derisk/core/
	# TODO: More package checks with mypy.

.PHONY: coverage
coverage: setup ## Run tests and report coverage
	$(VENV_BIN)/pytest derisk --cov=derisk
